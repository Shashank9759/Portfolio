(function () {
  ...
  function init() {
    if (window.__portfolioBgRunning) return;
    ...
    function profile() {
      ...
      if (mode === "tablet") {
        return {
          stars: 280, galaxy: 450, asteroids: 60, nebulae: 4, wormholes: 2, aliens: 8,
          sysScale: scale, warp: 0.018, miniSystems: 2,
        };
      }
      return {
        stars: 110, galaxy: 180, asteroids: 25, nebulae: 2, wormholes: 1, aliens: 4,
        sysScale: scale, warp: 0.012, miniSystems: 1,
      };
    }
    ...
    function drawGalaxy(now) {
      var th = theme(), pf = profile(), center = solarCenter();
    ...
    function drawWarpStars(now) {
      var th = theme(), pf = profile(), center = solarCenter(), speed = pf.warp;
    ...
    function drawSun(cx, cy, scale, alpha, now) {
      var th = theme(), r = 46 * scale, pulse = 1 + Math.sin(now * 0.0018) * 0.08;
    ...
    function drawSolarSystem(cx, cy, scale, alpha, now, planetSet, showLabels) {
      ...
      positions.forEach(function (item) {
        ...
        if (showLabels && (mode === "desktop" || mode === "tablet") && pos.z > 0.05) {
          ctx.fillStyle = rgba(th.particle, 0.82 * pos.alpha * alpha);
    ...
    function drawAliens(now) {
      var th = theme(), pf = profile(), sun = solarCenter();
    ...
    resize();
    window.__portfolioBgRunning = true;
    requestAnimationFrame(frame);
  }

  window.portfolioBackgroundInit = init;
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bootBackground, { once: true });
  } else {
    bootBackground();
  }
})();
